...
public class MainGame {
    ...
    private ArrayList<GameState> gameStore;
    ...
    private void addRandomTile() {
        ...
        if (grid.isCellsAvailable()) {
            //int value = Math.random() < 0.9 ? 2 : 4; *** OG ***
            int value;
            double rnnd = Math.random();
            if(rnnd < 0.2) {
                value = 2;
            } else if(rnnd < 0.4) {
                value = 8;
    ...
    private void saveUndoState() {
        ...
        gameStore.add(new GameState(grid, bufferScore, bufferGameState));
    }
    ...
    private void prepareUndoState() {
        ...
        gameStore.add(new GameState(grid, score, gameState));
    }
    ...
    public void revertUndoState() {
        ...
        if(gameStore.size() > 0) {
            aGrid.cancelAnimations();
            GameState tempState = gameStore.get(gameStore.size() - 1);
            grid = tempState.getGrid();
            score = tempState.getScore();
            gameState = tempState.getState();
            mView.refreshLastTime = true;
            mView.invalidate();
            gameStore.remove(gameStore.size() - 1); // Delete from storage
        }
    }
